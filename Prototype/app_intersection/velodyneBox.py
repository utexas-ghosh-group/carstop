...
import prototype.app_intersection.point2rect
...
vert_cos = cos(vert_angle*pi/180)*distance_convert
vert_sin = sin(vert_angle*pi/180)*distance_convert
...
def processLidarPacket(msg, last_angle = 0, lasers=None):
    xyz = np.empty((384,3))
    ...
    for k in range(12):
        ...
        mid = start+16
        xyz[start:mid,0] = fires[:16] * hcos * vert_cos
        xyz[mid:end,0] = fires[16:32] * hcos2 * vert_cos
        xyz[start:mid,1] = fires[:16] * -hsin * vert_cos
        xyz[mid:end,1] = fires[16:32] * -hsin2 * vert_cos
        xyz[start:end,2] = fires * vert_sin
        keep[start:end] = (fires > min_distance_to_keep) &\
                             (fires < max_distance_to_keep)
        if not (lasers is None):
            keep[start:end] &= lasers
